...
if (isset($_GET['hapus'])) {
    ...
    $stmt->execute();
    header("Location: adminrt.php?tampil=rwtampil&pesan=hapus");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_rw'])) {
    ...
    if ($nomor_rw && $nama_rw && $alamat_rw) {
        ...
        $stmt->execute();
        header("Location: adminrt.php?tampil=rwtampil&pesan=update");
        exit;
    } else {
...
<div class="mb-4 flex justify-center">
    <a href="adminrt.php?tampil=rwtambah" class="inline-block bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition">
        ...
        <tbody>
            ...
            <tr class="hover:bg-gray-100">
                ...
                <td class="px-4 py-2 border text-center space-x-2">
                    <a href="adminrt.php?tampil=rwtampil&edit=<?= $row['id_rw'] ?>" class="inline-block bg-yellow-400 text-white px-2 py-1 rounded hover:bg-yellow-500 transition">
                        <i class="fas fa-edit"></i> Edit
                    </a>
                    <a href="adminrt.php?tampil=rwtampil&hapus=<?= $row['id_rw'] ?>" onclick="return confirm('Yakin ingin menghapus data ini?')" 
